...
const OrderMyTrades = () => {
  ...
  const cleanupTradeOrder = useCallback(
    async (privateKey: string, zkAccount: ZkAccount) => {
      if (!twilightAddress) {
        return {
          success: false,
  ...
  const handleSettleOrder = useCallback(
    async (tradeOrder: TradeOrder) => {
      ...
      const output =
        transactionHashRes.data.result.find(
      ...
      try {
        ...
        if (!newAccountBalance) {
          console.error(
            "settling order failed to get balance",
            newAccountBalance
          );
          toast({
            variant: "error",
    ...
    async (tradeOrder: TradeOrder) => {
      ...
      try {
        ...
        if (
          ...
          result.result.includes("not cancelable")
        ) {
          toast({
            variant: "error",
            title: "Error",
        ...
        const cleanupResult = await cleanupTradeOrder(
          privateKey,
          currentAccount
        );
  ...
  const enhancedColumns = useMemo(() => {
    return myTradesColumns.map((column) => {
      ...
      if (
        ...
        column.accessorKey === "calculatedUnrealizedPnl"
      ) {
        return {
          ...column,
